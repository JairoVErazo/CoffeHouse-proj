...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`/api/Productos/categoria${categoria}`);
  ...
  const agregarProducto = (producto) => {
    const nuevoProducto = { ...productosSeleccionados };
    nuevoProducto[producto.idProducto] = (nuevoProducto[producto.idProducto] || 0) + 1;
    setProductosSeleccionados(nuevoProducto);
  };

  const quitarProducto = (producto) => {
    const nuevoProducto = { ...productosSeleccionados };
    if (nuevoProducto[producto.idProducto] > 1) {
      nuevoProducto[producto.idProducto]--;
    } else {
      delete nuevoProducto[producto.idProducto];
    }
    setProductosSeleccionados(nuevoProducto);
  };
  ...
  return (
    <div className="flex flex-col items-center py-10">
      <ul className="font-bold flex p-4 mt-2 space-x-24 flex-row text-rose-900 text-4xl mb-20 list-disc cursor-pointer">
        <li
          className={`hover:text-pink-950 ${categoria === 1 ? "bg-pink-200" : ""}`}
          onClick={() => setCategoria(1)}
        >
          Iced Coffee
        </li>
        <li
          className={`hover:text-pink-950 ${categoria === 2 ? "bg-pink-200" : ""}`}
          onClick={() => setCategoria(2)}
        >
          Hot Coffee
        </li>
        <li
          className={`hover:text-pink-950 ${categoria === 3 ? "bg-pink-200" : ""}`}
          onClick={() => setCategoria(3)}
        >
          Dessert
        </li>
        <li
          className={`hover:text-pink-950 ${categoria === 4 ? "bg-pink-200" : ""}`}
          onClick={() => setCategoria(4)}
        >
          Temporada
        </li>
        <li
          className={`hover:text-pink-950 ${categoria === 5 ? "bg-pink-200" : ""}`}
      ...
      <div className="">
        <div className="flex ml-12" style={{ maxWidth: "1300px", overflowX: "scroll" }}>
          {datos.length === 0 ? (
            ...
            datos.map((producto) => (
              <div className="pe-14 relative mb-20" key={producto.idProducto}>
                <div className="absolute top-0 left-2 cursor-pointer">
                  <button onClick={() => router.push("/Menu/detalles/" + producto.idProducto)}>
                    <img src="info.svg" alt="Icono Modal" className="h-8" />
                  </button>
                </div>
                <div className="w-72 h-auto border border-gray-200 rounded-lg shadow">
                  <a href="#">
                    <img src="img/menu1.png" alt="" className="h-70 rounded-t-lg" />
                  </a>
                  <div className="p-5 bg-white">
                    ...
                    <div className="flex justify-center px-10 cursor-pointer">
                      ...
                      {productosSeleccionados[producto.idProducto] && (
                        <button
                      ...
                      <div className="px-3 text-rose-900 text-2xl">
                        {productosSeleccionados[producto.idProducto] || 0}
                      </div>
                    </div>
                  </div>
                </div>
